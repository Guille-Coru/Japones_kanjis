...
def check_answer(clicked):
    ...
    intentos = st.session_state.intentos[st.session_state.target]
...
pct = (100 * st.session_state.total_aciertos / st.session_state.total_clicks) if st.session_state.total_clicks > 0 else 0
st.write(f"✅ Acierto: {pct:.1f}%")

if st.session_state.target is None:
    st.success("¡Completado!")
else:
    st.subheader("Significado del kanji objetivo")
